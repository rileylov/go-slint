...
// generate produces the typed Go source for a component interface. files holds
// every transitively-imported local .slint (keyed relative to the entry's
// directory) so a multi-file component compiles from the embedded source with no
// files on disk. relPath supplies the entry .slint's base name. The embedded
// markup is the single source of truth; `goslint generate` refreshes it.
func generate(iface *Interface, pkg, style, source, relPath string, files map[string]string) ([]byte, error) {
	...
	// Embedded markup, compiled once. This is the source of truth: editing the
	// .slint and re-running `goslint generate` (which goslint dev/build/run do
	// automatically) refreshes it.
	p("var generatedSource = %s\n\n", strconv.Quote(source))
	if multi {
		p("// generatedEntryName is the entry .slint's base name; the embedded file-loader\n")
		p("// treats imports as relative to it.\n")
